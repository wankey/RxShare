...
class WeiboHandler(activity: Activity) : ShareHandler(activity) {
    ...
    init {
        WbSdk.install(activity, AuthInfo(activity, SocialConfig.instance.weiboId, SocialConfig.instance.weiboRedirectUrl, SocialConfig.instance.weiboScope))
    ...
    override fun share(target: SocialMedia, model: ShareModel) {
        if (!isInstall()) {
            BusUtils.default.post(ShareResult(activity.getString(R.string.msg_weibo_client_not_found)))
            activity.finish()
            return
        }
    ...
    override fun handleResult(data: Intent?) {
        api.doResultIntent(data, object : WbShareCallback {
            override fun onWbShareSuccess() {
                BusUtils.default.post(ShareResult(activity.getString(R.string.action_share_success)))
            }

            override fun onWbShareCancel() {
                BusUtils.default.post(ShareResult(activity.getString(R.string.action_share_cancel)))
            }

            override fun onWbShareFail() {
                BusUtils.default.post(ShareResult(activity.getString(R.string.action_share_failure)))
            }
        })
    }
